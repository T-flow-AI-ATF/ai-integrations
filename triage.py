...
if __name__ == "__main__":
    ...
    async def main():
        """Example usage of the triage system."""
        print("=== T-Flow AI Medical Triage System ===")
        
        # Test AI triage
        result1 = await triage_patient("Patient has severe chest pain and difficulty breathing", {"age": 45}, True)
        print(f"AI Triage: {result1['triage_level']}")
        
        # Test rule-based triage
        result2 = await triage_patient("Patient has mild headache", {"age": 30}, False)
        print(f"Rule-based Triage: {result2['triage_level']}")
        
        # Test vitals
        vitals_result = await flag_vitals({"pulse": 110, "systolicBP": 180, "diastolicBP": 95})
